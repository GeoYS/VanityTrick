...
import rtsgame.components.ingame.victoryconditions.DestinationCondition;
 ...
 * @author GeoYS_2
 */
public class FirstLevelState extends BaseGameState{

    public static int ID = 353;
    private MessageBox introduction;
    private DestinationCondition vc;
    ...
    public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
        ...
        introduction = new MessageBox("Fact: " + Facts.getRandomFact() + " Campaign 1. "
                + "Objective: Move your units towards the base by making a selection box"
                + " around them and right-clicking on or beside the base."
                + "Good Luck! ", gc, gc.getWidth() / 4, gc.getHeight() / 4);
        ArrayList<Unit> utt = new ArrayList();
        Vector2f d = null;
        for(Unit u : units){
            if(u.getType() == Unit.BUILDING){
                d = u.getLocation();
            }
            else if(u.getType() == Unit.UNIT){
                utt.add(u);
            }
        }
        vc = new DestinationCondition(utt, d);
    }
    ...
    private void loadLevel() throws FileNotFoundException{
        Scanner fin = new Scanner(new File("level1.txt"));
